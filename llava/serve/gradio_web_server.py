...
def change_input_mode(current_mode):
    if current_mode == "image":
        return "video", gr.update(visible=False), gr.update(visible=True)
    else:
        return "image", gr.update(visible=True), gr.update(visible=False)
...
def add_text(state, text, image, video, image_process_mode, request: gr.Request):
    ...
    if image is not None:
        ...
        if '<image>' not in text:
            text = text + '\n<image>\n<deepfake>'
            # text = '<image>\n<deepfake>\n' + text
        # if '<deepfake>' not in text:
        #     text = f'<deepfake> {text}'
            ...
            with gr.Column(scale=3):
                with gr.Row(elem_id="model_selector_row"):
                    model_selector = gr.Dropdown(
                        choices=models,
                        value='ours' if len(models) > 0 else "",
                        interactive=True,
                        show_label=False,
                        container=False)

                imagebox = gr.Image(type="pil", visible=True)
                videobox = gr.Video(label="Upload Video", visible = False)  # Add video upload component
